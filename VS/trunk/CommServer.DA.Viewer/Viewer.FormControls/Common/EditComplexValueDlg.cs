...
namespace CAS.Lib.OPCClientControlsLib
{
  ...
  public class EditComplexValueDlg: System.Windows.Forms.Form
  {
    ...
    public EditComplexValueDlg()
    {
      ...
      TypeTV.ImageList = ImageListLibrary.StaticProjectImageList;
      FieldsLV.SmallImageList = ImageListLibrary.StaticProjectImageList;
    ...
    private void InitializeComponent()
    {
      ...
      this.FieldsLV.ColumnClick += new ColumnClickEventHandler( SortHandler.listView_ColumnClick );
      // 
      // Splitter01
      // 
    ...
    private class BinaryValue
    {
      ...
      {
        return new BinaryReader().Read( data, m_dictionary, m_typeID );
      }

      // serializes the buffer according to the schema.
      public byte[] Parse( ComplexValue complexValue )
      {
        return new BinaryWriter().Write( complexValue, m_dictionary, m_typeID );
      }
    }
    ...
    private void ShowFormattedValue( TreeNodeCollection nodes, ComplexValue complexValue, int index )
    {
      ...
      if ( node == null )
      {
        ...
        node.ImageIndex = (int)ImageListLibrary.Icons.IMAGE_CLOSED_YELLOW_FOLDER;
        node.SelectedImageIndex = (int)ImageListLibrary.Icons.IMAGE_CLOSED_YELLOW_FOLDER;
      ...
      if ( complexValue.Value.GetType() == typeof( ComplexValue[] ) )
      {
        ...
        foreach ( ComplexValue element in (System.Array)complexValue.Value )
        {
    ...
    void ShowComplexValue( ComplexValue complexValue )
    {
      ...
      {
        ListViewItem listItem = new ListViewItem( complexValue.Name, (int)ImageListLibrary.Icons.IMAGE_TAG );
      ...
      ArrayList fields = new ArrayList( (System.Array)complexValue.Value );
      ...
      for ( int ii = 0; ii < fields.Count; ii++ )
      {
        ...
        if ( fields[ ii ].GetType() == typeof( ComplexValue ) )
        {
          ...
          listItem = new ListViewItem( elementValue.Name, (int)ImageListLibrary.Icons.IMAGE_TAG );
          ...
          if ( elementValue.Value.GetType() == typeof( ComplexValue[] ) )
          {
            ...
            listItem.SubItems.Add( System.String.Format( "ComplexValue[{0}]", ( (System.Array)elementValue.Value ).Length ) );
          }
        ...
        {
          listItem = new ListViewItem( System.String.Format( "[{0}]", ii ), (int)ImageListLibrary.Icons.IMAGE_TAG );
      ...
      if ( rawValue.GetType() == typeof( byte[] ) )
      {
        ...
        for ( int ii = 0; ii < data.Length; ii++ )
        {
          ...
          if ( pos == 0 )
          {
            ...
            buffer.Append( System.String.Format( "{0:X4} ", ii ) );
          }

          buffer.Append( System.String.Format( "{0:X2} ", data[ ii ] ) );
          chars[ pos ] = System.Convert.ToChar( data[ ii ] );

          if ( System.Char.IsControl( chars[ pos ] ) )
          {
            chars[ pos ] = '.';
          }
        }
    ...
    private string FormatXml( string xml )
    {
      try
      {
        ...
        System.IO.StringWriter ostrm = new System.IO.StringWriter();
