	...
	public class EditArrayDlg : System.Windows.Forms.Form
	{
      ...
      if ( value == null )
        throw new System.ArgumentNullException( "value" );
			if (!value.GetType().IsArray) throw new System.ArgumentException("Not an array", "value");
		...
		private new void Update()
		{
			...
			foreach (object element in (System.Array)m_value)
			{
				string[] columns = new string[] 
				{ 
					System.String.Format("[{0}]", index++), 
					Convert.ToString(element)
				};

				//TODO Select appropriate icone
        ListViewItem item = new ListViewItem(columns, (int)ImageListLibrary.Icons.IMAGE_TAG);
				item.Tag = element;
		...
		private bool Save()
		{
			...
			try
			{
				// get andOpc.Convert the new value.
        object value =Opc.Convert.ChangeType( ValueCTRL.Get(), m_type );

				// update the array.
				((System.Array)m_value).SetValue(value, m_index);
		...
		private void ViewMI_Click(object sender, System.EventArgs e)
		{
			if (ArrayLV.SelectedItems.Count == 1)
			{
				...
				try
				{
					...
					// update the array.
					((System.Array)m_value).SetValue(value, index);
		...
		private void InsertMI_Click(object sender, System.EventArgs e)
		{
			if (!m_readOnly && ArrayLV.SelectedIndices.Count > 0)
			{
				...
				{
					array.Add(Convert.Clone(template));
				}
				...
				//Opc.Convert to array with correct element type.
