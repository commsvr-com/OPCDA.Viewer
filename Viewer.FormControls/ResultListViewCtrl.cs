//<summary>
//  Title   : A control used to display a set of results.
...
namespace CAS.Lib.OPCClientControlsLib
{
  ...
  public class ResultListViewCtrl: System.Windows.Forms.UserControl
  {
    ...
    /// </summary>
    public void Initialize( Server server, string locale, Opc.IdentifiedResult[] results )
    {
      Initiaize( server, locale );
    ...
    /// </summary>
    public void Initialize( OpcDa::ItemResult[] results )
    {
      Initiaize( null, null );
      // check if there is nothing to do.
      if ( results == null || results.Length == 0 )
        return;
      foreach ( OpcDa::ItemResult result in results )
        AddResult( result );
      // adjust the list view columns to fit the data.
      AdjustColumns();
    }
    ...
    /// </summary>
    public void Initialize( Server server, string locale, OpcDa::ItemValueResult[] results )
    {
      Initiaize( server, locale );
      // check if there is nothing to do.
      if ( results == null || results.Length == 0 )
    ...
    /// </summary>
    private Hashtable m_errors = new Hashtable();
    private void Initiaize( Server server, string locale )
    {
      m_server = server;
      m_locale = locale;
      m_errors.Clear();
    ...
    {
      if ( item.GetType() == typeof( Opc.IdentifiedResult ) )
        return GetFieldValue( (Opc.IdentifiedResult)item, fieldID );
      if ( item.GetType() == typeof( OpcDa::ItemResult ) )
        return GetFieldValue( (OpcDa::ItemResult)item, fieldID );
      if ( item.GetType() == typeof( OpcDa::ItemValueResult ) )
        return GetFieldValue( (OpcDa::ItemValueResult)item, fieldID );
      return null;
    }
    /// <summary>
    /// Returns the value of the specified field.
    /// </summary>
    private object GetFieldValue( Opc.IdentifiedResult item, int fieldID )
    {
      switch ( fieldID )
      {
    ...
    /// </summary>
    private void AddResult( Opc.IResult item )
    {
      // create list view item.
      ListViewItem listItem = new ListViewItem( (string)GetFieldValue( item, ITEM_NAME ), (int)ImageListLibrary.Icons.IMAGE_TAG );
    ...
    private void ItemListLV_MouseDown( object sender, System.Windows.Forms.MouseEventArgs e )
    {
      ...
      if ( ItemListLV.SelectedItems.Count == 1 )
        ViewMI.Enabled = ( clickedItem.Tag != null && clickedItem.Tag.GetType() == typeof( OpcDa::ItemValueResult ) );
    }
    ...
    private void ShowErrorTextMI_Click( object sender, System.EventArgs e )
    {
      ...
      foreach ( ListViewItem listItem in ItemListLV.Items )
      {
        ...
        if ( listItem.Tag.GetType() == typeof( Opc.IdentifiedResult ) )
          errorText = GetErrorText( ( (Opc.IdentifiedResult)listItem.Tag ).ResultID );
        else if ( listItem.Tag.GetType() == typeof( OpcDa::ItemResult ) )
          errorText = GetErrorText( ( (OpcDa::ItemResult)listItem.Tag ).ResultID );
        else if ( listItem.Tag.GetType() == typeof( OpcDa::ItemValueResult ) )
          errorText = GetErrorText( ( (OpcDa::ItemValueResult)listItem.Tag ).ResultID );
